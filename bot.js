...
client.on("message", async message => {
  if(message.channel.id !== "947112924441346098" || message.author.bot) return;
  iltifatSayi++
  if(iltifatSayi >= 150) { // 10 yazan yer, 10 mesajda bir iltifat edeceğini gösterir, değiştirebilirsiniz.
    iltifatSayi = 0;
    const senju = Math.floor(Math.random() * ((iltifatlar).length - 1) + 1);
    message.reply(`**${(iltifatlar)[senju]}**`);
  };
});
...
client.on("message", message => {
    ...
    return message.channel.send(`**Brahman\n #0034\n 神**`)
});
...
client.on("message", message => {
    ...
    return message.channel.send(`${message.author}, Merhaba Kardeş.`)
});
 ...
 if(!db.has(`reklam_${msg.guild.id}`)) return;
        ...
        if (reklam.some(word => msg.content.includes(word))) {
          try {
            if (!msg.member.hasPermission("BAN_MEMBERS")) {
                  msg.delete();
                    return msg.channel.send(new Discord.MessageEmbed().setDescription(`${msg.author} Bu sunucuda reklam yapamazsın kardeşim.`).setColor('0x800d0d').setAuthor(msg.member.displayName, msg.author.avatarURL({ dynamic: true })).setTimestamp()).then(x => x.delete({timeout: 5000}));
